...
**Source File:** `admin-COMPLETE.html`  
**Total Lines:** 1,924  
            ...
            <div class="control-panel">
                ...
                <div class="section">
                    ...
                    <div class="upload-area">
                        ...
                        <input type="text" class="editable-text upload-subtext" id="admin-upload-subtext" value="Will be resized to 75×75 pixels" style="text-align: center;">
                    </div>
                </div>
                ...
                <div class="section">
                    ...
                    <div class="grid-options">
                        <button class="grid-btn" data-size="48" id="admin-grid-btn-48" contenteditable="true" style="cursor: text; padding: 12px; border: 1px solid #333333; background: #E0E0E0; border-radius: 6px; font-size: 14px; font-weight: normal; color: #333333; transition: all 0.2s;">48 × 48</button>
                        <button class="grid-btn active" data-size="75" id="admin-grid-btn-75" contenteditable="true" style="cursor: text; padding: 12px; border: 1px solid #333333; background: #E0E0E0; border-radius: 6px; font-size: 14px; font-weight: normal; color: #333333; transition: all 0.2s;">75 × 75</button>
                        <button class="grid-btn" data-size="96" id="admin-grid-btn-96" contenteditable="true" style="cursor: text; padding: 12px; border: 1px solid #333333; background: #E0E0E0; border-radius: 6px; font-size: 14px; font-weight: normal; color: #333333; transition: all 0.2s;">96 × 96</button>
                    </div>
                </div>
                ...
                <div class="section" id="price-display-section" style="margin-top: 30px;">
                    <div style="margin-bottom: 15px;">
                        ...
                        <div class="price-details">
                            <div class="price-detail-row">
                                ...
                                <input type="text" class="price-detail-input" id="admin-dimensions" value="75 × 75" readonly>
                            </div>
                            <div class="price-detail-row">
                    ...
                    <div style="margin-top: 15px;">
                        <div class="price-detail-row">
                            <span class="price-detail-label" id="admin-label-48x48" contenteditable="true" style="cursor: text; padding: 1px 3px; border-radius: 3px; transition: background-color 0.2s; display: inline-block;">48×48:</span>
                            <input type="number" step="0.01" min="0" id="admin-price-48x48" class="price-detail-input" value="39.99">
                        </div>
                        <div class="price-detail-row">
                            <span class="price-detail-label" id="admin-label-75x75" contenteditable="true" style="cursor: text; padding: 1px 3px; border-radius: 3px; transition: background-color 0.2s; display: inline-block;">75×75:</span>
                            <input type="number" step="0.01" min="0" id="admin-price-75x75" class="price-detail-input" value="49.99">
                        </div>
                        <div class="price-detail-row">
                            <span class="price-detail-label" id="admin-label-96x96" contenteditable="true" style="cursor: text; padding: 1px 3px; border-radius: 3px; transition: background-color 0.2s; display: inline-block;">96×96:</span>
                ...
                <div class="section" id="stl-section" style="margin-top: 30px;">
                    ...
                    <div class="stl-section">
                        <div class="stl-item">
                            <span class="stl-label">48×48 STL:</span>
                        ...
                        <div class="stl-item">
                            <span class="stl-label">75×75 STL:</span>
                            <div style="display: flex; align-items: center;">
                        ...
                        <div class="stl-item">
                            <span class="stl-label">96×96 STL:</span>
                            <div style="display: flex; align-items: center;">
                ...
                <div class="info-box" style="background: transparent; border: none; padding: 20px; margin: 20px 0; text-align: left; max-width: 100%; box-sizing: border-box;">
                    ...
                    <textarea class="editable-textarea" id="admin-info-description" style="font-size: 14px; line-height: 1.6; color: #666; margin-bottom: 15px; text-align: left; width: 100%;">Turn your favourite photos into stunning brick art—made by you!</textarea>
...
Once you're happy with your design, click "Proceed to Checkout" to order your custom brick mosaic.</textarea>
    ...
    <script>
        ...
        const BACKEND_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' 
            ? 'http://localhost:5000' 
            : 'https://backend-admim.netlify.app/'; // ⚠️ REPLACE THIS with your Render backend URL (e.g., 'https://rize-albums-backend.onrender.com')
        ...
        if (BACKEND_URL.includes('YOUR_BACKEND_URL_HERE') || BACKEND_URL.includes('your-backend-url')) {
            console.error('⚠️ BACKEND_URL is not configured! Please update line 1083 in admin.html with your actual backend URL.');
            // Show error on page load
            setTimeout(() => {
                const statusEl = document.getElementById('status-message');
                if (statusEl) {
                    statusEl.className = 'status-message error';
                    statusEl.textContent = '⚠️ ERROR: Backend URL not configured! Deploy server.py to Render.com, then update BACKEND_URL in admin.html (line ~1083) with your Render backend URL. Example: https://your-app-name.onrender.com';
        ...
        function getApiUrl(path) {
            ...
            if (BACKEND_URL.includes('YOUR_BACKEND_URL_HERE') || BACKEND_URL.includes('your-backend-url')) {
                showStatus('error', '⚠️ Backend URL not configured! Deploy server.py to Render.com, then update BACKEND_URL in admin.html with your Render backend URL.');
        ...
        async function loadOrders() {
            try {
                ...
                document.getElementById('orders-list').innerHTML = `<p style="color: red;">Failed to load orders: ${error.message}</p>`;
            }
        }
                    ...
                    <div class="order-item" style="${isCompleted ? 'opacity: 0.6;' : ''}" data-order-id="${order.order_id}" data-completed="${isCompleted}">
                        <div class="order-header">
                            <div style="display: flex; align-items: center; gap: 15px;">
                                ...
                                <div>
                                    <div class="order-id">Order #${order.order_id.substring(0, 8)} ${isCompleted ? '<span style="color: #2d5016; font-size: 12px;">✓ Completed</span>' : ''}</div>
                                    <div class="order-date">${formattedDate}</div>
                                </div>
                            </div>
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <div class="order-price">$${order.total_price.toFixed(2)}</div>
                                <button onclick="deleteOrder('${order.order_id}')" class="delete-order-btn" title="Delete Order">🗑️</button>
                            </div>
                        </div>
                        ...
                        <div class="order-files">
                            <a href="/admin/orders/download/${order.order_id}/model.obj" class="file-download-btn">Download OBJ</a>
                        </div>
                    </div>
                `;
            }).join('');
        ...
        async function uploadSTL(size, file) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    showStatus('success', `STL file for ${size}×${size} uploaded successfully!`);
                } else {
                    statusEl.textContent = 'Upload failed';
                    statusEl.style.color = '#d32f2f';
            ...
            try {
                ...
                for (const size of [48, 75, 96]) {
                    ...
                    if (statuses[`${size}x${size}`]) {
                        statusEl.textContent = 'File exists ✓';
                        statusEl.style.color = '#2d5016';
                    }
                }
            } catch (error) {
