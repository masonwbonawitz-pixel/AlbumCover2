...
**Source File:** `mobile-index-COMPLETE.html`  
**Total Lines:** 5,542  
...
This is the **EXACT** code from the file. Search through it to find any function, class, style, or element.
        ...
        <div class="control-panel">
            ...
            <div class="section collapsible-section" id="size-panel">
                ...
                <div class="grid-options">
                    <button class="grid-btn" data-size="48" id="mobile-grid-btn-48" onclick="selectGridSize(48)">Normal</button>
            ...
            <div class="top-action-buttons" id="action-buttons-container" style="display: none;">
                <label for="png-input" class="top-action-btn" id="upload-btn" style="cursor: pointer; -webkit-tap-highlight-color: transparent; touch-action: manipulation;">
            ...
            <input type="file" id="png-input" class="file-input" accept="image/*,.png,.jpg,.jpeg,.heic,.heif" />
        </div>
    ...
    <script>
        ...
        async function loadPrices() {
            try {
                ...
                selectedGridSize = 75;
                // Set noise reduction based on default grid size (75 = 0)
        ...
        let editorSettings = {
            ...
            blackPoint: 0,
            whitePoint: 255,
            noiseReduction: 0  // Auto-set: 5 for 48x48, 0 for 75x75 and 96x96
        };
        // Crop variables
        ...
        function createProcessedImageAtSize(image, targetSize, callback) {
            const isPosterizedMode = (targetSize === 48);
            
            // Cap resolution at 2048x2048 for Normal mode to prevent lag
            ...
            // Cap resolution for posterized mode to prevent lag
            if (isPosterizedMode) {
                const aspect = sourceWidth / sourceHeight;
                if (sourceWidth > MAX_PROCESSING_SIZE || sourceHeight > MAX_PROCESSING_SIZE) {
                    if (sourceWidth > sourceHeight) {
                        sourceWidth = MAX_PROCESSING_SIZE;
                        sourceHeight = Math.round(MAX_PROCESSING_SIZE / aspect);
                    } else {
                        sourceHeight = MAX_PROCESSING_SIZE;
                        sourceWidth = Math.round(MAX_PROCESSING_SIZE * aspect);
                    }
                }
            }

            // For posterized mode, process at capped resolution first, then resize
            // For pixelated mode, process at target size directly
            ...
            // Draw image at processing resolution
            if (isPosterizedMode) {
                processCtx.drawImage(imageToProcess, 0, 0, sourceWidth, sourceHeight);
            } else {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                // For posterized mode, always use 4 colors; otherwise use settings
                const base = [0,85,170,255];
                const n = isPosterizedMode ? 4 : Math.max(2, Math.min(4, editorSettings.tones|0));
                const indices = [];
                for (let k = 0; k < n; k++) {
            ...
            // For posterized mode, resize to target size after processing
            // For pixelated mode, use processed canvas directly
            ...
            if (isPosterizedMode) {
                // Enable smoothing when resizing posterized image
                finalCtx.imageSmoothingEnabled = true;
                finalCtx.drawImage(processCanvas, 0, 0, targetSize, targetSize);
            } else {
                // Disable smoothing for pixelated mode
        ...
        async function selectGridSize(size) {
            ...
            // Set noise reduction based on grid size
            // 48x48 (Normal) = 5, others (75, 96) = 0
            if (size === 48) {
                editorSettings.noiseReduction = 5;
            } else {
                editorSettings.noiseReduction = 0;
            }
        ...
        async function show3DView() {
            ...
            if (rawUploadedImage && currentMesh) {
                if (processedImageData && processedImageData.width > 0 && processedImageData.height > 0) {
                    // Use painted image data if available (full resolution for Normal mode)
        ...
        // Fast non-blurring denoising using mode filter
        // Only replaces isolated noise pixels, preserves all edges and details
        function applyNoiseReduction(imageData, strength) {
            ...
            // Small 3x3 window for speed - only process isolated noise pixels
            const windowSize = 3;
            const halfWindow = 1;
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    const centerColor = originalData[centerIdx];
                    
                    // Quick check: count how many neighbors match the center
                    let matchingNeighbors = 0;
                    let totalNeighbors = 0;
                    const neighborColors = [];
                    
                    // Sample 8 neighbors (3x3 minus center)
                    for (let dy = -halfWindow; dy <= halfWindow; dy++) {
                        for (let dx = -halfWindow; dx <= halfWindow; dx++) {
                            ...
                            if (nx >= 0 && nx < width && ny >= 0 && ny < height) {
                                ...
                                const neighborColor = originalData[idx];
                    ...
                    // Only process if this pixel is likely noise (few matching neighbors)
                    // This preserves edges and only fixes isolated noise
                    if (matchingNeighbors < totalNeighbors * 0.3) {
                        // Find most common color in neighborhood (mode)
                        const colorCounts = {};
                        ...
                        // Only replace if dominant color is clearly the majority
                        if (maxCount >= totalNeighbors * 0.4) {
                            data[centerIdx] = dominantColor;
                            data[centerIdx + 1] = dominantColor;
                            data[centerIdx + 2] = dominantColor;
                        }
                    }
                }
            }
        ...
        function processImage() {
            ...
            // Create temp canvas - full resolution (capped) for posterized mode, pixel size for others
            ...
            if (cropCoordinates && cropCoordinates.size > 0) {
                ...
                if (isPosterizedMode) {
                    // Draw cropped image at capped resolution (no pixelation)
                    tempCtx.drawImage(cropCanvas, 0, 0, sourceWidth, sourceHeight);
                } else {
                ...
                if (isPosterizedMode) {
                    // Draw full image at capped resolution (no pixelation)
                    tempCtx.drawImage(rawUploadedImage, 0, 0, sourceWidth, sourceHeight);
                } else {
            ...
            // Apply efficient noise reduction if enabled
            const noiseReduction = editorSettings.noiseReduction ?? 0;
            if (noiseReduction > 0) {
                const denoiseImageData = tempCtx.getImageData(0, 0, tempCanvas.width, tempCanvas.height);
                applyNoiseReduction(denoiseImageData, noiseReduction);
                tempCtx.putImageData(denoiseImageData, 0, 0);
            }
            ...
            previewCtx.imageSmoothingEnabled = isPosterizedMode;
        ...
        function redrawProcessedCanvas() {
            ...
            if (!processedCanvas) return;
            const processedCtx = processedCanvas.getContext('2d');
            const isPosterizedMode = (selectedGridSize === 48);
        ...
        function paintPixelAt(clickX, clickY, processedCanvas) {
            ...
            if (isPosterizedMode) {
                // Circular brush for posterized mode
                const radius = brushSize / 2;
                const radiusSq = radius * radius;
                
                // Paint pixels within circular area
            ...
            return new Promise((resolve, reject) => {
                ...
                reader.onload = function(event) {
                    try {
                        ...
                        // For Normal mode, we'll use texture mapping with no lighting; for others, use vertex colors with lighting
                        // Material will be set in applyColorsToMesh based on mode
                        const isNormalMode = (selectedGridSize === 48);
                        let material;
                        
                        if (isNormalMode) {
                            // Normal mode: Use texture mapping with MeshBasicMaterial (no lighting)
                            material = new THREE.MeshBasicMaterial({
                                color: 0xffffff,
                                side: THREE.DoubleSide
                            });
                        } else {
                            // Pixelated modes: Use vertex colors with MeshStandardMaterial (with lighting)
        ...
        function applyColorsToMesh() {
            ...
            // For Normal mode (48), use texture mapping (UV coordinates)
            // For other modes, use vertex colors
            const isNormalMode = (selectedGridSize === 48);
            
            if (isNormalMode) {
                // Normal mode: Generate UV coordinates and apply texture
                const uvs = new Float32Array(positions.count * 2);
                
                for (let i = 0; i < positions.count; i++) {
                    const v = vertices[i];
                    // Normalize XY coordinates to [0, 1] range
                    const u = (v.x - minX) / sizeX;
                    const v_coord = 1.0 - (v.y - minY) / sizeY; // Flip Y: top of STL maps to top of image
                ...
                // Create texture from the processed/painted image
                // Use processedImageData if available (contains user's painted edits), otherwise use pngImage
                let textureSource;
                if (processedImageData && processedImageData.width > 0 && processedImageData.height > 0) {
                    // Create canvas from processed image data (includes user's painted edits)
                    const textureCanvas = document.createElement('canvas');
                    textureCanvas.width = processedImageData.width;
                    textureCanvas.height = processedImageData.height;
                    const textureCtx = textureCanvas.getContext('2d');
                    textureCtx.putImageData(processedImageData, 0, 0);
                    textureSource = textureCanvas;
                    console.log('✅ Creating texture from processedImageData:', textureCanvas.width, 'x', textureCanvas.height);
                } else if (pngImage) {
                    // Fallback: use pngImage directly
                    textureSource = pngImage;
                    console.log('✅ Creating texture from pngImage:', pngImage.width, 'x', pngImage.height);
                } else {
                    console.error('❌ No image source available for texture mapping');
                    return;
                }
                
                // Create texture from canvas or image
                const texture = new THREE.CanvasTexture(textureSource);
                texture.flipY = false; // Don't flip Y since we already flipped in UV coordinates
                ...
                console.log('✅ Applied texture mapping to mesh (Normal mode)');
                return; // Done for Normal mode
            }

            // Pixelated modes: Use vertex colors
            ...
            // isNormalMode is already declared at the top of this function
            currentMesh.material.vertexColors = true;
            currentMesh.material.map = null; // Remove texture if it exists
            currentMesh.material.needsUpdate = true;
            
            // For Normal mode, no lighting/shading needed
            // For pixelated modes, keep smooth shading to show geometry details
            if (!isNormalMode) {
        ...
        function triggerFileInput() {
            ...
            if (fileInput) {
                // Use direct click (works on all mobile browsers)
                fileInput.click();
            }
        }

        // Add explicit touch/click handlers for upload buttons (mobile compatibility)
        window.addEventListener('DOMContentLoaded', () => {
            const fileInput = document.getElementById('png-input');
            if (!fileInput) return;

            // Handler for initial upload box
            const initialUploadBox = document.getElementById('initial-upload-box');
            if (initialUploadBox) {
                // Use touchstart - this fires before click on mobile
                initialUploadBox.addEventListener('touchstart', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    triggerFileInput();
                }, { passive: false });
                // Click handler for mouse/desktop - will be blocked if touchstart fired recently
                initialUploadBox.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    triggerFileInput();
                });
            ...
            // Handler for upload button in toolbar
            ...
            // Handler for upload area in panel
            const uploadArea = document.getElementById('png-upload-area');
            if (uploadArea) {
                // Use touchstart - this fires before click on mobile
        ...
        document.getElementById('add-to-cart-btn').addEventListener('click', async () => {
            ...
            try {
                ...
                // For Normal mode, use full resolution processedImageData; otherwise use grid size
                if (processedImageData && processedImageData.width > 0 && processedImageData.height > 0) {
                    const tempCanvas = document.createElement('canvas');
                    if (isNormalMode) {
                        // Use full resolution for Normal mode
                        tempCanvas.width = processedImageData.width;
                        tempCanvas.height = processedImageData.height;
                    } else {
                        // Use grid size for pixelated modes
                        tempCanvas.width = selectedGridSize;
                        tempCanvas.height = selectedGridSize;
                    }
                    const tempCtx = tempCanvas.getContext('2d');
                    
                    // Put the processed image data directly (full resolution for Normal mode)
                    tempCtx.putImageData(processedImageData, 0, 0);
                    ...
                    // Fallback: use processed canvas if processedImageData not available
                    const processedCanvas = document.getElementById('processed-canvas');
                    if (processedCanvas && processedCanvas.width > 0) {
                        const tempCanvas = document.createElement('canvas');
                        tempCanvas.width = isNormalMode ? processedCanvas.width : selectedGridSize;
                        tempCanvas.height = isNormalMode ? processedCanvas.height : selectedGridSize;
                        const tempCtx = tempCanvas.getContext('2d');
                        tempCtx.imageSmoothingEnabled = isNormalMode;
