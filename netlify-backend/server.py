...
def write_3mf_with_texture(vertices: np.ndarray, faces: np.ndarray, img_rgb: np.ndarray, png_bytes: bytes) -> bytes:
    ...
    try:
        ...
        # Set texture content (PNG bytes)
        if hasattr(texture2d, "SetContent"):
            texture2d.SetContent(png_bytes)
        elif hasattr(texture2d, "SetAttachment"):
            texture2d.SetAttachment(png_bytes)
        else:
            # Try to set via path or other method
        ...
        if hasattr(mesh_obj, "SetUVCoordinates"):
            ...
            for u, v_coord in uvs:
                uv_list.append((float(u), float(v_coord)))
            mesh_obj.SetUVCoordinates(uv_list)
        elif hasattr(mesh_obj, "SetTextureCoordinates"):
            mesh_obj.SetTextureCoordinates([(float(u), float(v)) for u, v in uvs])
        
        # Link texture to mesh via material
        ...
        print(f"✅ Added texture2d resource (ID: {texture_id}) to 3MF")
    except Exception as e:
        print(f"⚠️  Warning: Could not add texture to 3MF: {e}")
        print(f"   Falling back to per-triangle colors...")
        # Fallback to per-triangle colors
        triangle_colors = get_triangle_colors_from_image(vertices, faces, img_rgb, img_rgb.shape[0])
        return write_3mf(vertices, faces, triangle_colors)
...
def write_obj_with_colors(vertices: np.ndarray, faces: np.ndarray, triangle_colors: np.ndarray, is_normal_mode: bool = False, img_rgb: np.ndarray = None) -> Tuple[bytes, bytes]:
    """
    Write OBJ file with vertex colors (works for both Normal and pixelated modes)
    Uses vertex colors (v x y z r g b) format - same approach that works for pixelated modes
    ...
    if is_normal_mode:
        ...
        print(f"✅ Creating OBJ with exact vertex colors (4-color palette)")
    ...
    obj_content.append("# Vertex colors format for Bambu Studio compatibility")
    obj_content.append("# Vertices duplicated per triangle to prevent color interpolation")
    obj_content.append("")
    
    # Duplicate vertices for each triangle so each triangle gets exact color
    # This prevents color interpolation when vertices are shared
    new_vertices = []
    new_faces = []
    vertex_offset = 0
    
    for face_idx, face in enumerate(faces):
        tri_color = triangle_colors[face_idx] / 255.0  # Normalize to 0-1
        r, g, b = tri_color[0], tri_color[1], tri_color[2]
        ...
        # Add vertices with exact color for this triangle
        new_vertices.append((v0, r, g, b))
        new_vertices.append((v1, r, g, b))
        new_vertices.append((v2, r, g, b))
        
        # Create new face indices (1-indexed for OBJ)
        new_faces.append((vertex_offset + 1, vertex_offset + 2, vertex_offset + 3))
        vertex_offset += 3
    ...
    # For pixelated modes (not Normal mode), ensure all 4 colors are present
    # This helps Bambu Studio recognize all 4 colors for 4-color printing
    if not is_normal_mode:
        ...
        for i, color in enumerate(FOUR_COLORS_RGB):
            ...
            if color_tuple in unique_colors:
                ...
                print(f"   ✗ {color_names[i]}: {color} (MISSING - will add dummy triangle)")
        
        # Ensure all 4 colors are present by adding tiny dummy triangles if needed
        if len(unique_colors) < 4:
        bbox_min = vertices.min(axis=0)
        bbox_max = vertices.max(axis=0)
        bbox_size = bbox_max - bbox_min
        dummy_offset = np.array([bbox_size[0] * 1.1, bbox_size[1] * 1.1, bbox_size[2] * 1.1])  # Place outside model
        
        for i, color in enumerate(FOUR_COLORS_RGB):
            color_tuple = tuple(color)
            if color_tuple not in unique_colors:
                print(f"   ➕ Adding dummy triangle for {color_names[i]} to ensure all colors are present")
                # Create a tiny triangle far from the model
                r, g, b = color[0] / 255.0, color[1] / 255.0, color[2] / 255.0
                v_dummy = bbox_min + dummy_offset + np.array([i * 0.001, 0, 0])  # Slight offset per color
                
                # Add 3 vertices for a tiny triangle
                new_vertices.append((v_dummy, r, g, b))
                new_vertices.append((v_dummy + np.array([0.0001, 0, 0]), r, g, b))
                new_vertices.append((v_dummy + np.array([0, 0.0001, 0]), r, g, b))
                
                new_faces.append((vertex_offset + 1, vertex_offset + 2, vertex_offset + 3))
                vertex_offset += 3
    
    # Write vertices with colors (v x y z r g b)
    for v, r, g, b in new_vertices:
    ...
    # Return empty MTL bytes since we're using vertex colors
    mtl_bytes = b""
    return obj_bytes, mtl_bytes


def generate_3mf_from_inputs(stl_bytes: bytes, png_bytes: bytes, grid_size: int = 75) -> bytes:
    # Use per-triangle colors for ALL modes (same as 3D viewer uses vertex colors for all modes)
...
def generate_obj_route():
    ...
    try:
        ...
        # For Normal mode (48x48), use UV texture mapping (baking image to texture)
        # For pixelated modes, use vertex colors
        ...
        try:
            ...
            if os.path.exists(prices_file):
                with open(prices_file, 'r') as f:
                    prices = json.load(f)
                    price_key = f"{grid_size}x{grid_size}"
                    base_price = prices.get(price_key, 0.0)
        except Exception as e:
            print(f"⚠️  Warning: Could not load price: {e}")
        
        # For Normal mode (48x48), generate OBJ with UV texture (baking image to texture)
...
def admin_prices_api():
    """Admin API to get/edit prices"""
    prices_file = 'prices.json'
    
    if request.method == 'GET':
        # Return current prices as JSON
        try:
            if os.path.exists(prices_file):
                with open(prices_file, 'r') as f:
                    prices = json.load(f)
        ...
        # Update prices (admin only - add password check if needed)
        try:
...
def get_prices():
    """Public API to get current prices (read-only for customers)"""
    prices_file = 'prices.json'
    try:
        if os.path.exists(prices_file):
            with open(prices_file, 'r') as f:
...
def admin_content_api():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if os.path.exists(content_file):
                with open(content_file, 'r') as f:
                    ...
                    # Merge saved content into defaults (saved content overrides defaults)
                    content.update(saved_content)
        ...
        try:
            ...
            merged_content = default_content.copy()
