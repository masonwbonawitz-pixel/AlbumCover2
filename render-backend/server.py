...
def write_obj_with_colors(vertices: np.ndarray, faces: np.ndarray, triangle_colors: np.ndarray) -> Tuple[bytes, bytes]:
    """
    Write OBJ file with vertex colors for Bambu Studio compatibility
    Duplicates vertices so each triangle has exact colors without interpolation
    Uses vertex colors (v x y z r g b) format
    Returns: (obj_bytes, mtl_bytes) - mtl_bytes is empty for vertex color OBJ
    """
    print(f"✅ Creating OBJ with exact vertex colors (no interpolation)")
    ...
    # Check which colors are actually used and ensure all 4 are present
    unique_colors = set()
    for tri_color in triangle_colors:
        color_tuple = tuple(tri_color)
    ...
    # This helps Bambu Studio recognize all 4 colors
...
# Flask app
app = Flask(__name__)
# Enable CORS for frontend access - allow all origins for admin panel and all API routes
CORS(app, resources={
    r"/admin/*": {"origins": "*"},
    r"/api/*": {"origins": "*"},
    r"/product_images/*": {"origins": "*"},
    r"/get-stl/*": {"origins": "*"},
    r"/upload-for-checkout": {"origins": "*"},
    r"/generate": {"origins": "*"},
    r"/generate-obj": {"origins": "*"}
})
...
def mobile(timestamp=None):
    """Serve mobile version (for local dev) or return API message (production)"""
    # Check if mobile/index.html exists locally (for local development)
    ...
    if os.path.exists(mobile_path):
        ...
        return response
    # Production: return API message
    return jsonify({
        'message': 'Frontend is served by Netlify. This is the backend API only.',
        'status': 'backend_api'
    }), 404

@app.route('/desktop')
def desktop():
    """Serve desktop version (for local dev) or return API message (production)"""
    # Check if desktop.html exists locally (for local development)
...
def admin_prices_page():
    """Serve admin panel (for local dev) or return API message (production)"""
    # Check if admin.html exists locally (for local development)
    admin_path = os.path.join(os.path.dirname(__file__), '..', 'admin.html')
    if os.path.exists(admin_path):
        return send_file(admin_path)
    # Production: return API message
    return jsonify({
        'message': 'Admin panel is served by Netlify. This is the backend API only.',
        'status': 'backend_api',
        'api_endpoints': {
            'prices': '/admin/prices/api',
            'images': '/admin/images/api',
            'stl': '/admin/stl/api',
            'content': '/admin/content/api',
